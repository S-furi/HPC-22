...
server:
        ...
        wget
https://www.moreno.marzolla.name/teaching/HPC/handouts/omp-brute-force.c
...
#pragma omp parallel default(none) shared(out, enc, check, CHECK_LEN, found, msglen, nmax, KEY_LEN)
  {
    const int my_id = omp_get_thread_num();
    const int thread_count = omp_get_num_threads();
    const int my_start = (nmax * my_id) / thread_count;
    const int my_end = (nmax * (my_id + 1)) / thread_count;
    char *my_out = (char *)malloc(msglen);
    char my_key[KEY_LEN + 1];

    for(int k = my_start; !found && k < my_end ; k++) {
      snprintf(my_key, KEY_LEN + 1, "%08d", k);
      xorcrypt(enc, my_out, msglen, my_key, KEY_LEN);

      if(0 == memcmp(my_out, check, CHECK_LEN)) {
        found++;
        printf("Key found: %s\n", my_key);
