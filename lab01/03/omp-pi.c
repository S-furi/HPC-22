...
#pragma omp parallel default(none) shared(inside, num_threads, n_points)

  {
    const int my_id = omp_get_thread_num();
    const int my_start = (my_id * n_points) / num_threads;
    const int my_end = (n_points * (my_id + 1)) / num_threads;
    inside[my_id] = generate_points(my_end - my_start);
  }
