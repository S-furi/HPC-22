...
int main( int argc, char* argv[] )
{
    ...
    int sendcnts[comm_sz], displs[comm_sz], local_start, local_end;


    for (int i = 0; i < comm_sz; i++) {
        const int local_start = n * i / comm_sz;
        const int local_end = n * (i + 1) / comm_sz;

        displs[i] = local_start;
        sendcnts[i] = local_end - local_start;
    }

    const int local_n = sendcnts[my_rank];
    double *local_x = (double*)malloc(local_n * sizeof(*local_x)); assert(local_x != NULL);
    double *local_y = (double*)malloc(local_n * sizeof(*local_y)); assert(local_y != NULL);


    MPI_Scatterv(x, sendcnts, displs, MPI_DOUBLE, local_x, local_n, MPI_DOUBLE, 0, MPI_COMM_WORLD);
    MPI_Scatterv(y, sendcnts, displs, MPI_DOUBLE, local_y, local_n, MPI_DOUBLE, 0, MPI_COMM_WORLD);
    
    const double local_result = dot(local_x, local_y, local_n);

    printf("Proc:%d computed %f\n", my_rank, local_result);
