...
While discussing parallel programming patterns, we introduced Cellular
Automata (CAs) as an example of _stencil computations_. In this
exercise we implement the [Rule 30 Cellular
...
assume cyclic boundary conditions where the neighbors of $x[0]$ are
...
Note that the sequence □□□■■■■□ = 00011110 on the second row is the
binary representation of decimal 30, from which the name ("Rule 30
CA"); more details can be found [here](mpi-rule30.pdf).

The file [mpi-rule30.c](mpi-rule30.c) contains a serial program that
computes the evolution of the Rule 30 CA. Initially, all cells are 0
except the central one. The program accepts two optional command line
parameters: the domain size $N$ and the number of steps to
simulate. At the end, process 0 produces the image `rule30.pbm` shown
in Figure 2.
...
int main( int argc, char* argv[] )
{
    ...
    /* fanno riferimento al dominio globale */
    ...
    /* The master distributes the domain cur[] to the other MPI
      ...
      const int LOCAL_LEFT = LOCAL_LEFT_GHOST + 1;
      const int LOCAL_RIGHT = LOCAL_LEFT + local_width - 1;
      const int LOCAL_RIGHT_GHOST = LOCAL_RIGHT + 1;

      /* cur[] comprende la ghost area, ricordiamo che non e' da mettere */
      /* &cur[LEFT] indice del primo vero elemento */
      MPI_Scatter( &cur[LEFT]           /* sendbuf */,
                   local_width          /* sendcount */,
                   MPI_CHAR             /* datatype */,
                   &local_cur[LEFT]     /* recvbuf */,
    ...
    for (s=0; s<steps; s++) {
        ...
        */
        ...
        /* Gather the updated local domains at the root; it is
           ...
           all. */
        /* non includere nella gather le ghost areas */
