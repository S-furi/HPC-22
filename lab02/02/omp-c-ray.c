...
the [recursive algorithm of
Whitted](http://citeseerx.ist.psu.edu/viewdoc/summary?doi=10.1.1.156.1534)
(Figure 2).
...
void render(int xsz, int ysz, pixel_t *fb, int samples)
{
     ...
     */
#pragma omp parallel for default(none) collapse(2) shared(fb, samples, xsz, ysz)
    for (j=0; j<ysz; j++) {
        for (i=0; i<xsz; i++) {
            double r, g, b;
            int s;
