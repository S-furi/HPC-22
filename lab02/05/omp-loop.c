...
#pragma omp parallel default(none) shared(a, n, b) private(i)
  {
#pragma omp for
    for (i = 1; i < n; i++) {
      b[i] = a[i - 1];
...
#pragma omp parallel default(none) \
  shared(a, n, num_threads, rightmost)
  {
    const int my_id = omp_get_thread_num();
    const int my_start = my_id * n / num_threads;
    const int my_end = (my_id + 1) * n / num_threads;
...
void test1_seq(int *A, int n) {
  ...
  for (i = 1; i < n; i++) {
    for (j = 1; j < n - 1; j++) {
      ...
      A[IDX(i, j, n)] = f(A[IDX(i - 1, j - 1, n)], A[IDX(i - 1, j, n)],
                          A[IDX(i - 1, j + 1, n)]);
    }
  }
}
...
void test1_par(int *A, int n) {
  ...
  int i, j;
  for (i = 1; i < n; i++) {
#pragma omp parallel for default(none) shared(i, A, n)
    for (j = 1; j < n - 1; j++) {
      A[IDX(i, j, n)] = f(A[IDX(i - 1, j - 1, n)], A[IDX(i - 1, j, n)],
                          A[IDX(i - 1, j + 1, n)]);
    }
...
void test2_par(int *A, int n) {
  ...
  int i, j;
  for (j = 1; j < n; j++) {
#pragma omp parallel for default(none) shared(j, A, n)
    for (i = 1; i < n; i++) {
      /*
        A[i][j] = g(A[i,j-1], A[i-1,j-1])
       */
...
void test3_seq(int *A, int n) {
  ...
  for (i = 1; i < n; i++) {
    for (j = 1; j < n; j++) {
       ...
       */
      A[IDX(i, j, n)] =
          f(A[IDX(i, j - 1, n)], A[IDX(i - 1, j - 1, n)], A[IDX(i - 1, j, n)]);
    }
  }
}
...
#pragma omp parallel for default(none) shared(A, n, z, slice) private(i, j)
    for(i = slice - z; i >= z; i--) {
      j = slice - i;
      if (i == 0 || j == 0) {
        continue;
